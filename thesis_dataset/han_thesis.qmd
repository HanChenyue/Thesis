...
# lme4 code for treatment and diagnosis interaction: BASMI

```{r}
# Make sure that the reference level for treatment_reclassified is "Standard Non-High Cost Drug Treatment"
dataframe_sorted_and_reclassified_basmi$treatment_reclassified <- relevel(dataframe_sorted_and_reclassified_basmi$treatment_reclassified, ref = "Standard Non-High Cost Drug Treatment")

# Make sure that the reference level for hlba27status is "undiagnosed"
dataframe_sorted_and_reclassified_basmi$hlab27status <- relevel(dataframe_sorted_and_reclassified_basmi$hlab27status, ref = "undiagnosed")

# Make sure that the reference level for diagnosis is "Axial SpA- nr-axSpA"
dataframe_sorted_and_reclassified_basmi$diagnosis <- relevel(dataframe_sorted_and_reclassified_basmi$diagnosis, ref = "Axial SpA- AS")

# Make sure that the reference level for treatment_order is 0
dataframe_sorted_and_reclassified_basmi$treatment_order <- relevel(dataframe_sorted_and_reclassified_basmi$treatment_order, ref = 1)

# Make sure that the reference level for last_known_smoking_status is "no information"
dataframe_sorted_and_reclassified_basmi$last_known_smoking_status <- relevel(dataframe_sorted_and_reclassified_basmi$last_known_smoking_status, ref = "no information")

# Make sure that the reference level for last_known_vaping_status is "no information"
dataframe_sorted_and_reclassified_basmi$last_known_vaping_status <- relevel(dataframe_sorted_and_reclassified_basmi$last_known_vaping_status, ref = "no information")

# Define the full and reduced models
model_1_full <- lmer(basmi_score ~ gender + hlab27status + age_at_basmi + diagnosis + treatment_order + last_known_smoking_status + last_known_vaping_status + treatment_reclassified*diagnosis + time_in_years + (time_in_years | ttas_id), data = dataframe_sorted_and_reclassified_basmi,
                         control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))
                      )
model_1_reduced <- lmer(basmi_score ~ gender + hlab27status + age_at_basmi + treatment_order + last_known_smoking_status + last_known_vaping_status + treatment_reclassified + diagnosis + time_in_years + (time_in_years | ttas_id), data = dataframe_sorted_and_reclassified_basmi,
                         control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))
                         )
```

```{r}
print(summary(model_1_full, corr = FALSE))
```

```{r}
print(summary(model_1_reduced, corr = FALSE))
```
...
model_1_reduced_vif <- vif(model_1_reduced)
model_1_full_vif
model_1_reduced_vif
```

```{r, include=FALSE}
# # Old code: Attempts to make the output as image
# # Capture the full model summary output as text
# model_1_summary_capture <- capture.output(print(summary(model_1_full, corr = FALSE)))
# 
# # Convert the summary text into an HTML preformatted block
# html_summary <- paste("<pre>", paste(model_1_summary_capture, collapse = "\n"), "</pre>")
# 
# # Save the HTML version
# writeLines(html_summary, "model_1_summary.html")
# 
# # Use webshot to convert the HTML file to an image
# webshot("model_1_summary.html", "model_1_summary_table_image.png")
```
