...
# Daily Activity only
# General longitudinal analysis for those with and without drugs
dataframe_sorted_daily_activity <- dataframe_sorted %>%
  filter(!is.na(daily_activity)) %>%
        select(ttas_id, gender, hlab27status, age_at_prom, diagnosis, treatment, last_known_smoking_status, last_known_vaping_status, treatment_order, date, stop_date_for_period_date_format, overall_treatment_duration_in_days, treatment_window_weekly_time, time_in_days, time_in_weeks, time_in_months, time_in_years, daily_activity_impairment)

write.csv(dataframe_sorted_daily_activity, "temp_dataframe_after_dataframe_sorted_for_analysis_daily_activity.csv", row.names = FALSE)
```

```{r, include=FALSE}
# Ignore if there is only one instance of row per treatment_period_unique_identifier in each ttas_id
dataframe_sorted_daily_activity <- dataframe_sorted_daily_activity %>%
  group_by(ttas_id, treatment_order) %>%
  filter(n() > 1) %>% # Keep only groups with more than one row after filtering out NAs
  ungroup()

write.csv(dataframe_sorted_daily_activity, "temp_dataframe_after_dataframe_sorted_for_analysis_filtered.csv", row.names = FALSE)

# Reclassify the drugs
dataframe_sorted_and_reclassified_daily_activity <- dataframe_sorted_daily_activity %>%
  mutate(treatment_reclassified = case_when(
    treatment %in% c("Amgevita", "Humira", "Imraldi", "Yuflyma", "Benepali", "Enbrel", "Simponi", "Remicade", "Remsima", "Cimzia") ~ "Anti-TNF",
    treatment %in% c("Cosentyx", "Taltz", "Bimzelx") ~ "Anti-IL17",
    treatment %in% c("Rinvoq", "Xeljanz") ~ "JAK Inhibitor",
...
write.csv(dataframe_sorted_and_reclassified_daily_activity, "temp_dataframe_after_dataframe_sorted_for_analysis_filtered_with_treatment_category_daily_activity.csv", row.names = FALSE)

# If diagnosis is "Undifferentiated spondyloarthritis (uSpA) (17 ttas_id)" and "Axial SpA- No subgroup" (1 ttas_id), remove it as most of them are on standard treatment and is their representation is limited
dataframe_sorted_and_reclassified_daily_activity <- dataframe_sorted_and_reclassified_daily_activity %>%
  filter(diagnosis != "Undifferentiated spondyloarthritis (uSpA)" & diagnosis != "Axial SpA- No subgroup")

write.csv(dataframe_sorted_and_reclassified_daily_activity, "temp_dataframe_after_dataframe_sorted_for_analysis_filtered_with_treatment_category_and_diagnosis_daily_activity.csv", row.names = FALSE)
...
```{r, include=FALSE}
# Ensure that dataframe_sorted_and_reclassified_daily_activity has the correct data types
dataframe_sorted_and_reclassified_daily_activity$ttas_id <- as.factor(dataframe_sorted_and_reclassified_daily_activity$ttas_id)
dataframe_sorted_and_reclassified_daily_activity$gender <- as.factor(dataframe_sorted_and_reclassified_daily_activity$gender)
dataframe_sorted_and_reclassified_daily_activity$hlab27status <- as.factor(dataframe_sorted_and_reclassified_daily_activity$hlab27status)
dataframe_sorted_and_reclassified_daily_activity$age_at_prom <- as.numeric(dataframe_sorted_and_reclassified_daily_activity$age_at_prom)
dataframe_sorted_and_reclassified_daily_activity$diagnosis <- as.factor(dataframe_sorted_and_reclassified_daily_activity$diagnosis)
dataframe_sorted_and_reclassified_daily_activity$treatment <- as.factor(dataframe_sorted_and_reclassified_daily_activity$treatment)
dataframe_sorted_and_reclassified_daily_activity$treatment_order <- as.factor(dataframe_sorted_and_reclassified_daily_activity$treatment_order)
dataframe_sorted_and_reclassified_daily_activity$treatment_reclassified <- as.factor(dataframe_sorted_and_reclassified_daily_activity$treatment_reclassified)
dataframe_sorted_and_reclassified_daily_activity$date <- as.Date(dataframe_sorted_and_reclassified_daily_activity$date)
dataframe_sorted_and_reclassified_daily_activity$stop_date_for_period_date_format <- as.Date(dataframe_sorted_and_reclassified_daily_activity$stop_date_for_period_date_format)
dataframe_sorted_and_reclassified_daily_activity$overall_treatment_duration_in_days <- as.numeric(dataframe_sorted_and_reclassified_daily_activity$overall_treatment_duration_in_days)
dataframe_sorted_and_reclassified_daily_activity$treatment_window_weekly_time <- as.numeric(dataframe_sorted_and_reclassified_daily_activity$treatment_window_weekly_time)
dataframe_sorted_and_reclassified_daily_activity$daily_activity_impairment <- as.numeric(dataframe_sorted_and_reclassified_daily_activity$daily_activity_impairment)
dataframe_sorted_and_reclassified_daily_activity$time_in_days <- as.numeric(dataframe_sorted_and_reclassified_daily_activity$time_in_days)
dataframe_sorted_and_reclassified_daily_activity$time_in_weeks <- as.numeric(dataframe_sorted_and_reclassified_daily_activity$time_in_weeks)
dataframe_sorted_and_reclassified_daily_activity$time_in_months <- as.numeric(dataframe_sorted_and_reclassified_daily_activity$time_in_months)
dataframe_sorted_and_reclassified_daily_activity$time_in_years <- as.numeric(dataframe_sorted_and_reclassified_daily_activity$time_in_years)
dataframe_sorted_and_reclassified_daily_activity$last_known_smoking_status <- as.factor(dataframe_sorted_and_reclassified_daily_activity$last_known_smoking_status)
dataframe_sorted_and_reclassified_daily_activity$last_known_vaping_status <- as.factor(dataframe_sorted_and_reclassified_daily_activity$last_known_vaping_status)
```

```{r, include=FALSE}
# Make sure that the reference level for treatment_reclassified is
# "Standard Non-High Cost Drug Treatment"
dataframe_sorted_and_reclassified_daily_activity$treatment_reclassified <-
  relevel(dataframe_sorted_and_reclassified_daily_activity$treatment_reclassified,
          ref = "Standard Non-High Cost Drug Treatment")

# Make sure that the reference level for hlba27status is "undiagnosed"
dataframe_sorted_and_reclassified_daily_activity$hlab27status <-
  relevel(dataframe_sorted_and_reclassified_daily_activity$hlab27status,
          ref = "undiagnosed")

# Make sure that the reference level for diagnosis is "Axial SpA- nr-axSpA"
dataframe_sorted_and_reclassified_daily_activity$diagnosis <-
  relevel(dataframe_sorted_and_reclassified_daily_activity$diagnosis,
          ref = "Axial SpA- AS")

# Make sure that the reference level for treatment_order is 1
dataframe_sorted_and_reclassified_daily_activity$treatment_order <-
  relevel(dataframe_sorted_and_reclassified_daily_activity$treatment_order,
          ref = 1)

# Make sure that the reference level for last_known_smoking_status is
# "no information"
dataframe_sorted_and_reclassified_daily_activity$last_known_smoking_status <-
  relevel(dataframe_sorted_and_reclassified_daily_activity$last_known_smoking_status,
          ref = "no information")

# Make sure that the reference level for last_known_vaping_status is
# "no information"
dataframe_sorted_and_reclassified_daily_activity$last_known_vaping_status <-
  relevel(dataframe_sorted_and_reclassified_daily_activity$last_known_vaping_status,
          ref = "no information")
```
...
model_treatment_diagnosis_daily_activity_impairment_full <- lmer(
  daily_activity_impairment ~ gender + hlab27status + age_at_prom +
    ...
    treatment_reclassified * diagnosis +
    time_in_years + (time_in_years | ttas_id),
                     data = dataframe_sorted_and_reclassified_daily_activity,
                     control = lmerControl(optimizer = "bobyqa",
                                           optCtrl = list(maxfun = 2e5)))
```
...
```{r, include=TRUE}
print(summary(model_treatment_daily_activity_impairment_full, corr = FALSE))
```

```{r, include=TRUE}
print(summary(model_treatment_daily_activity_impairment_reduced, corr = FALSE))
```

```{r, include=TRUE}
anova(model_treatment_daily_activity_impairment_full,
      model_treatment_daily_activity_impairment_reduced)
```
