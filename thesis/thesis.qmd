...
ggplot(basmi_sheet_data, aes(x = age_at_basmi, y = basmi_score)) +
  ...
  geom_jitter(alpha = 0.18) +
  geom_smooth(method = "loess", se = FALSE, colour = "red") +
  # geom_smooth(method = "lm", se = FALSE, colour = "blue") +
  labs(title = "BASMI Score against Age at BASMI Scatterplot with LOESS Curve", x = "Age at BASMI", y = "BASMI Score") +
  theme_bw()
...
#     scale_fill_manual(values = drug_colours, na.value = "gray") +
#     facet_wrap(~ ttas_id, ncol = 1, scales = "free_y") +
#     labs(title = paste("Timeline plot for TTAS ID:", current_id)) + theme_bw()
# 
#   file_name <- paste(output_folder, "/timeline_plot_", current_id, ".png")
#   ggsave(file_name, plot, width = 10, height = 6, dpi = 300)
# }
...
dataframe_sorted <- dataframe %>%
  select(ttas_id, x3, gender, hlab27status, age_at_onset, age_at_basmi, age_at_diag, age_at_prom, diagnosis, date, stop_date, last_known_smoking_status, last_known_vaping_status, basmi_score, basdai, basfi, patient_global, back_pain_at_night, back_pain_anytime, as_qol, jenkins_sleep_1, jenkins_sleep_2, jenkins_sleep_3, jenkins_sleep_4, wpl, daily_activity_impairment, presenteeism, absenteeism, facit, margolis)
...
dataframe_sorted$age_at_onset <- as.numeric(dataframe_sorted$age_at_onset)
...
```{r, include=FALSE, fig.width= 10, fig.height = 15}
data_tt1 <- dataframe_sorted_and_reclassified_basmi %>% 
  filter(ttas_id %in% c(1, 2, 3, 4))

# Extract the treatment type from the custom labels manually
data_tt1 <- dataframe_sorted_and_reclassified_basmi %>%
  filter(ttas_id %in% c(1, 2, 3, 4)) %>%
  mutate(
    treatment_group = case_when(
      ttas_id == 1 & treatment_order == 1 ~ "Standard Non-High Cost Drug Treatment",
      ttas_id == 1 & treatment_order == 2 ~ "Anti-TNF (Remicade)",
      ttas_id == 1 & treatment_order == 4 ~ "Anti-TNF (Remicade)",
      ttas_id == 2 & treatment_order == 1          ~ "Standard Non-High Cost Drug Treatment",
      ttas_id == 3 & treatment_order == 1          ~ "Standard Non-High Cost Drug Treatment",
      ttas_id == 3 & treatment_order == 2          ~ "Anti-TNF (Humira)",
      ttas_id == 3 & treatment_order == 4          ~ "Anti-TNF (Cimzia)",
      ttas_id == 3 & treatment_order == 6          ~ "Anti-IL17 (Cosentyx)",
      ttas_id == 4 & treatment_order == 1          ~ "Standard Non-High Cost Drug Treatment",
      ttas_id == 4 & treatment_order == 2          ~ "Anti-TNF (Humira)"
    )
  )

treatment_colors <- c(
  "Standard Non-High Cost Drug Treatment" = "#1f77b4",  # Blue
  "Anti-TNF (Remicade)" = "#FFD300",                   # Yellow
  "Anti-TNF (Humira)" = "#2ca02c",                     # Green
  "Anti-IL17 (Cosentyx)" = "#d62728",                  # Red
  "Anti-TNF (Cimzia)" = "#9467bd"                      # Purple
)


# custom_labels <- c(
# "1.1" = "Patient 1, Standard Non-High Cost Drug Treatment",
# "1.2" = "Patient 1, Anti-TNF (Remicade)",
# "1.4" = "Patient 1, Anti-TNF (Remicade)",
# "2.1" = "Patient 2, Standard Non-High Cost Drug Treatment",
# "3.1" = "Patient 3, Standard Non-High Cost Drug Treatment",
# "3.2" = "Patient 3, Anti-TNF (Humira)",
# "3.4" = "Patient 3, Anti-TNF (Cimzia)",
# "3.6" = "Patient 3, Anti-IL17 (Cosentyx)",
# "4.1" = "Patient 4, Standard Non-High Cost Drug Treatment",
# "4.2" = "Patient 4, Anti-TNF (Humira)"
# )
...
#          colour = "Treatment Group") +  # Update the legend title
# 
#   facet_wrap(~ttas_id, ncol = 1) +
#   # scale_colour_manual(labels = custom_labels, values = scales::hue_pal()(length(custom_labels))) +
#   scale_colour_manual(values = treatment_colors) +  # Apply consistent colors
...
data_tt1_patient_1_to_4_plots <- ggplot(data_tt1, aes(x = time_in_years, y = basmi_score,
                                                         colour = treatment_group)) +  # Change to treatment_group
                                      geom_point(size = 2.5) +
                                      geom_line(size = 1) +
                                      labs(title = "BASMI Score Over Time by Treatment Periods for Patient 1 to Patient 4", 
                                           x = "Time (Years)",
                                           ...
                                           colour = "Treatment Group") +  # Update the legend title
                                      facet_wrap(~ttas_id, ncol = 1) +
                                      scale_colour_manual(values = treatment_colors) +  # Now treatment_colors will be applied correctly
                                      theme(legend.position = "bottom",
                                            ...
                                            legend.box.just = "center",  # Center the legend horizontally
                                            legend.text = element_text(size = 22),
                                            legend.title = element_text(size = 18)
                                            ) +
                                      theme_bw()

data_tt1_patient_1_to_4_plots

# Save the plot with A4 dimensions in millimeters
ggsave(
  "basmi_score_over_time_by_treatment_periods_for_patient_1_to_patient_4.png", 
  width = 200, 
  height = 280, 
  units = "mm"
)
...
dataframe_sorted_and_reclassified_basmi$age_at_onset <- as.numeric(dataframe_sorted_and_reclassified_basmi$age_at_onset)
dataframe_sorted_and_reclassified_basmi$age_at_diag <- as.numeric(dataframe_sorted_and_reclassified_basmi$age_at_diag)
...
model_treatment_diagnosis_basmi_full <- lmer(
  basmi_score ~ gender + hlab27status + age_at_onset +
...
```{r, include = TRUE}
anova(model_treatment_diagnosis_basmi_full)
```
...
#         axis.title = element_text(size = 18),    # Axis title size
#         axis.text = element_text(size = 16),     # Axis text size
#         legend.box = "vertical",               # Legend box orientation
...
ggplot(data_tt1, aes(x = time_in_years, y = basmi_score,
                                      ...
                                      geom_point(size = 2) +
                                      geom_line(size = 0.75) +
                                        geom_line(aes(y = predicted_basmi_score_full), linetype = "dashed", size = 1.5, color = "red", alpha = 0.5) +
                                        geom_line(aes(y = predicted_basmi_score_reduced), linetype = "dotted", size = 1.5, color = "blue", alpha = 0.5) +
                                      labs(title = "BASMI Score Over Time by Treatment Periods for Patient 1 to Patient 4", 
                                           x = "Time (Years)",
                                           y = "BASMI Score", 
                                           colour = "Treatment Group") +  # Update the legend title
...
ggsave("basmi_score_with_lmer_lines_full_and_reduced.png",
  width = 200, 
  height = 280, 
  units = "mm")
...
dataframe_sorted_basdai <- dataframe_sorted %>%
  filter(!is.na(basdai)) %>%
  select(ttas_id, gender, hlab27status, age_at_onset, age_at_diag, age_at_prom, diagnosis, treatment, last_known_smoking_status, last_known_vaping_status, treatment_order, date, stop_date_for_period_date_format, overall_treatment_duration_in_days, treatment_window_weekly_time, time_in_days, time_in_weeks, time_in_months, time_in_years, basdai)
...
dataframe_sorted_and_reclassified_basdai$age_at_onset <- as.numeric(dataframe_sorted_and_reclassified_basdai$age_at_onset)
dataframe_sorted_and_reclassified_basdai$age_at_diag <- as.numeric(dataframe_sorted_and_reclassified_basdai$age_at_diag)
...
model_treatment_diagnosis_basdai_full <- lmer(
  basdai ~ gender + hlab27status + age_at_onset +
    diagnosis + treatment_order + last_known_smoking_status +
    last_known_vaping_status +  
    treatment_reclassified * diagnosis +
    time_in_years + (time_in_years | ttas_id),
...
```{r, include = TRUE}
anova(model_treatment_diagnosis_basdai_full, corr = FALSE)
```
...
model_treatment_gender_basdai_full <- lmer(
  basdai ~ hlab27status + age_at_onset +
    diagnosis + treatment_order + last_known_smoking_status +
    last_known_vaping_status +  
    treatment_reclassified * gender +
    time_in_years + (time_in_years | ttas_id),
...
dataframe_sorted_basfi <- dataframe_sorted %>%
  filter(!is.na(basfi)) %>%
  select(ttas_id, gender, hlab27status, age_at_onset, age_at_diag, age_at_prom, diagnosis, treatment, last_known_smoking_status, last_known_vaping_status, treatment_order, date, stop_date_for_period_date_format, overall_treatment_duration_in_days, treatment_window_weekly_time, time_in_days, time_in_weeks, time_in_months, time_in_years, basfi)
...
dataframe_sorted_and_reclassified_basfi$age_at_onset <- as.numeric(dataframe_sorted_and_reclassified_basfi$age_at_onset)
dataframe_sorted_and_reclassified_basfi$age_at_diag <- as.numeric(dataframe_sorted_and_reclassified_basfi$age_at_diag)
...
model_treatment_diagnosis_basfi_full <- lmer(
  basfi ~ gender + hlab27status + age_at_onset +
    diagnosis + treatment_order + last_known_smoking_status +
    last_known_vaping_status +  
    treatment_reclassified * diagnosis +
    time_in_years + (time_in_years | ttas_id),
...
dataframe_sorted_patient_global <- dataframe_sorted %>%
  ...
  select(ttas_id, gender, hlab27status, age_at_onset, age_at_prom, diagnosis, treatment, last_known_smoking_status, last_known_vaping_status, treatment_order, date, stop_date_for_period_date_format, overall_treatment_duration_in_days, treatment_window_weekly_time, time_in_days, time_in_weeks, time_in_months, time_in_years, patient_global)
...
dataframe_sorted_and_reclassified_patient_global$age_at_onset <- as.numeric(dataframe_sorted_and_reclassified_patient_global$age_at_onset)
...
model_treatment_diagnosis_patient_global_full <- lmer(
  patient_global ~ gender + hlab27status + age_at_onset +
    diagnosis + treatment_order + last_known_smoking_status +
    last_known_vaping_status +  
    treatment_reclassified * diagnosis +
    time_in_years + (time_in_years | ttas_id),
...
dataframe_sorted_and_reclassified_back_pain_at_night$age_at_onset <- as.numeric(dataframe_sorted_and_reclassified_back_pain_at_night$age_at_onset)
...
model_treatment_diagnosis_back_pain_at_night_full <- lmer(
  back_pain_at_night ~ gender + hlab27status + age_at_onset +
    diagnosis + treatment_order + last_known_smoking_status +
    last_known_vaping_status +  
    treatment_reclassified * diagnosis +
    time_in_years + (time_in_years | ttas_id),
...
dataframe_sorted_and_reclassified_back_pain_anytime$age_at_onset <- as.numeric(dataframe_sorted_and_reclassified_back_pain_anytime$age_at_onset)
...
model_treatment_diagnosis_back_pain_anytime_full <- lmer(
  back_pain_anytime ~ gender + hlab27status + age_at_onset +
    diagnosis + treatment_order + last_known_smoking_status +
    last_known_vaping_status +  
    treatment_reclassified * diagnosis +
    time_in_years + (time_in_years | ttas_id),
...
dataframe_sorted_asqol <- dataframe_sorted %>%
  filter(!is.na(as_qol)) %>%
    select(ttas_id, gender, hlab27status, age_at_onset, age_at_prom, diagnosis, treatment, last_known_smoking_status, last_known_vaping_status, treatment_order, date, stop_date_for_period_date_format, overall_treatment_duration_in_days, treatment_window_weekly_time, time_in_days, time_in_weeks, time_in_months, time_in_years, as_qol)
...
dataframe_sorted_and_reclassified_asqol$age_at_onset <- as.numeric(dataframe_sorted_and_reclassified_asqol$age_at_onset)
...
model_treatment_diagnosis_asqol_full <- lmer(
  as_qol ~ gender + hlab27status + age_at_onset +
    diagnosis + treatment_order + last_known_smoking_status +
    last_known_vaping_status +  
    treatment_reclassified * diagnosis +
    time_in_years + (time_in_years | ttas_id),
...
dataframe_sorted_and_reclassified_jenkins_sleep_1$age_at_onset <- as.numeric(dataframe_sorted_and_reclassified_jenkins_sleep_1$age_at_onset)
...
dataframe_sorted_and_reclassified_jenkins_sleep_2$age_at_onset <- as.numeric(dataframe_sorted_and_reclassified_jenkins_sleep_2$age_at_onset)
...
dataframe_sorted_and_reclassified_jenkins_sleep_3$age_at_onset <- as.numeric(dataframe_sorted_and_reclassified_jenkins_sleep_3$age_at_onset)
...
dataframe_sorted_and_reclassified_jenkins_sleep_4$age_at_onset <- as.numeric(dataframe_sorted_and_reclassified_jenkins_sleep_4$age_at_onset)
...
model_treatment_diagnosis_jenkins_sleep_1_full <- lmer(
  jenkins_sleep_1 ~ gender + hlab27status + age_at_onset +
    diagnosis + treatment_order + last_known_smoking_status +
    last_known_vaping_status +  
    treatment_reclassified * diagnosis +
    time_in_years + (time_in_years | ttas_id),
...
model_treatment_diagnosis_jenkins_sleep_2_full <- lmer(
  jenkins_sleep_2 ~ gender + hlab27status + age_at_onset +
    diagnosis + treatment_order + last_known_smoking_status +
    last_known_vaping_status +  
    treatment_reclassified * diagnosis +
    time_in_years + (time_in_years | ttas_id),
...
model_treatment_diagnosis_jenkins_sleep_3_full <- lmer(
  jenkins_sleep_3 ~ gender + hlab27status + age_at_onset +
    diagnosis + treatment_order + last_known_smoking_status +
    last_known_vaping_status +  
    treatment_reclassified * diagnosis +
    time_in_years + (time_in_years | ttas_id),
...
model_treatment_diagnosis_jenkins_sleep_4_full <- lmer(
  jenkins_sleep_4 ~ gender + hlab27status + age_at_onset +
    diagnosis + treatment_order + last_known_smoking_status +
    last_known_vaping_status +  
    treatment_reclassified * diagnosis +
    time_in_years + (time_in_years | ttas_id),
...
dataframe_sorted_wpl <- dataframe_sorted %>%
  filter(!is.na(wpl)) %>%
      select(ttas_id, gender, hlab27status, age_at_onset, age_at_prom, diagnosis, treatment, last_known_smoking_status, last_known_vaping_status, treatment_order, date, stop_date_for_period_date_format, overall_treatment_duration_in_days, treatment_window_weekly_time, time_in_days, time_in_weeks, time_in_months, time_in_years, wpl)
...
dataframe_sorted_and_reclassified_wpl$age_at_onset <- as.numeric(dataframe_sorted_and_reclassified_wpl$age_at_onset)
...
model_treatment_diagnosis_wpl_full <- lmer(
  wpl ~ gender + hlab27status + age_at_onset +
    diagnosis + treatment_order + last_known_smoking_status +
    last_known_vaping_status +  
    treatment_reclassified * diagnosis +
    time_in_years + (time_in_years | ttas_id),
...
dataframe_sorted_and_reclassified_daily_activity_impairment$age_at_onset <- as.numeric(dataframe_sorted_and_reclassified_daily_activity_impairment$age_at_onset)
...
model_treatment_diagnosis_daily_activity_impairment_full <- lmer(
  daily_activity_impairment ~ gender + hlab27status + age_at_onset +
    diagnosis + treatment_order + last_known_smoking_status +
    last_known_vaping_status +  
    treatment_reclassified * diagnosis +
    time_in_years + (time_in_years | ttas_id),
...
dataframe_sorted_and_reclassified_presenteeism$age_at_onset <- as.numeric(dataframe_sorted_and_reclassified_presenteeism$age_at_onset)
...
model_treatment_diagnosis_presenteeism_full <- lmer(
  presenteeism ~ gender + hlab27status + age_at_onset +
    diagnosis + treatment_order + last_known_smoking_status +
    last_known_vaping_status +  
    treatment_reclassified * diagnosis +
    time_in_years + (time_in_years | ttas_id),
...
dataframe_sorted_and_reclassified_absenteeism$age_at_onset <- as.numeric(dataframe_sorted_and_reclassified_absenteeism$age_at_onset)
...
model_treatment_diagnosis_absenteeism_full <- lmer(
  absenteeism ~ gender + hlab27status + age_at_onset +
    diagnosis + treatment_order + last_known_smoking_status +
    last_known_vaping_status +  
    treatment_reclassified * diagnosis +
    time_in_years + (time_in_years | ttas_id),
...
dataframe_sorted_facit <- dataframe_sorted %>%
  filter(!is.na(facit)) %>%
    select(ttas_id, gender, hlab27status, age_at_onset, age_at_prom, diagnosis, treatment, last_known_smoking_status, last_known_vaping_status, treatment_order, date, stop_date_for_period_date_format, overall_treatment_duration_in_days, treatment_window_weekly_time, time_in_days, time_in_weeks, time_in_months, time_in_years, facit)
...
dataframe_sorted_and_reclassified_facit$age_at_onset <- as.numeric(dataframe_sorted_and_reclassified_facit$age_at_onset)
...
model_treatment_diagnosis_facit_full <- lmer(
  facit ~ gender + hlab27status + age_at_onset +
    diagnosis + treatment_order + last_known_smoking_status +
    last_known_vaping_status +  
    treatment_reclassified * diagnosis +
    time_in_years + (time_in_years | ttas_id),
...
dataframe_sorted_margolis <- dataframe_sorted %>%
  filter(!is.na(margolis)) %>%
    select(ttas_id, gender, hlab27status, age_at_onset, age_at_prom, diagnosis, treatment, last_known_smoking_status, last_known_vaping_status, treatment_order, date, stop_date_for_period_date_format, overall_treatment_duration_in_days, treatment_window_weekly_time, time_in_days, time_in_weeks, time_in_months, time_in_years, margolis)
...
dataframe_sorted_and_reclassified_margolis$age_at_onset <- as.numeric(dataframe_sorted_and_reclassified_margolis$age_at_onset)
...
model_treatment_diagnosis_margolis_full <- lmer(
  margolis ~ gender + hlab27status + age_at_onset +
    diagnosis + treatment_order + last_known_smoking_status +
    last_known_vaping_status +  
    treatment_reclassified * diagnosis +
    time_in_years + (time_in_years | ttas_id),
...
```{r}
# Old code: French cohort study
# # Data for the forest plot
# forest_data <- data.frame(
#   Study = c("Tournadre et al. (2013) - Male", "Tournadre et al. (2013) - Female", 
#                    "RUH cohort - Male", "RUH cohort - Female", 
#                    "Existing Literature - Male", "Existing Literature - Female"),
#   BASFI = c(2.7, 3.3, 
#                  3.3 + (-1.45), 3.3,  # RUH cohort: female score is 3.3, male difference estimated around -1.45
#                  3.0, 3.0),  # No difference for existing literature
#   Lower_CI = c(2.7 - 2.1, 3.3 - 2.3, 
#                3.3 + (-1.53572), 3.3, 
#                3.0, 3.0),  # Confidence intervals or no difference
#   Upper_CI = c(2.7 + 2.1, 3.3 + 2.3, 
#                3.3 + (-1.36357), 3.3, 
#                3.0, 3.0)   # Confidence intervals or no difference
# )
# 
...
# ggplot(forest_data, aes(x = BASFI, y = Study)) +
#   geom_point(size = 3) +
#   geom_errorbarh(aes(xmin = Lower_CI, xmax = Upper_CI), height = 0.2) +
#   geom_vline(xintercept = 2.7, linetype = "dashed", color = "blue", size = 0.8) +  # Add vertical line at 0 for no difference
#   geom_vline(xintercept = 3.3, linetype = "dashed", color = "#C11C84", size = 0.8) +  # Add vertical line at 0 for no difference
#     geom_vline(data = dummy_lines, aes(xintercept = BASFI_Average, color = Gender),
#              linetype = "dashed", size = 0.8) +
#   # labs(title = "BASFI Scores by Gender and Study",
#   #      x = "BASFI Score",
#   #      y = "Study and Gender") +
#   xlim(0, 10) +  # Set appropriate limits for the BASFI score range
#   theme_bw()
# 
# # Save for A4 size in mm
# ggsave("forest_plot.png", width = 200, height = 150, units = "mm")
```

# Investigating Age at Onset/Age at Diagnosis and Diagnosis

```{r, include=TRUE}
model_onset_diagnosis_basmi_full <- lmer(
  basmi_score ~ gender + hlab27status + age_at_onset +
    diagnosis + treatment_order + last_known_smoking_status +
    last_known_vaping_status + treatment_reclassified +
    age_at_onset * diagnosis +
    time_in_years + (time_in_years | ttas_id),
                     data = dataframe_sorted_and_reclassified_basmi,
                     control = lmerControl(optimizer = "bobyqa",
                                           optCtrl = list(maxfun = 2e5)))

print(summary(model_onset_diagnosis_basmi_full, corr = FALSE))
```

```{r, include=TRUE}
model_onset_diagnosis_basmi_reduced <- lmer(
  basmi_score ~ gender + hlab27status + age_at_onset +
    diagnosis + treatment_order + last_known_smoking_status +
    last_known_vaping_status +
    treatment_reclassified +
...
print(summary(model_onset_diagnosis_basmi_reduced, corr = FALSE))
```

```{r, include = TRUE}
anova(model_onset_diagnosis_basmi_full, model_onset_diagnosis_basmi_reduced)
```

```{r, include = TRUE}
anova(model_onset_diagnosis_basmi_full)
```

```{r, include = TRUE}
model_onset_diagnosis_basdai_full <- lmer(
  basdai ~ gender + hlab27status + age_at_onset +
    diagnosis + treatment_order + last_known_smoking_status +
    last_known_vaping_status + treatment_reclassified +
    age_at_onset * diagnosis +
...
print(summary(model_onset_diagnosis_basdai_full, corr = FALSE))
```

```{r, include = TRUE}
model_onset_diagnosis_basdai_reduced <- lmer(
  basdai ~ gender + hlab27status + age_at_onset +
    diagnosis + treatment_order + last_known_smoking_status +
    last_known_vaping_status +
    treatment_reclassified +
...
print(summary(model_onset_diagnosis_basdai_reduced, corr = FALSE))
```

```{r, include = TRUE}
anova(model_onset_diagnosis_basdai_full, model_onset_diagnosis_basdai_reduced)
```

```{r, include = TRUE}
anova(model_onset_diagnosis_basdai_full)
```

```{r, include = TRUE}
model_onset_diagnosis_basfi_full <- lmer(
  basfi ~ gender + hlab27status + age_at_onset +
    diagnosis + treatment_order + last_known_smoking_status +
    last_known_vaping_status + treatment_reclassified +
    age_at_onset * diagnosis +
...
print(summary(model_onset_diagnosis_basfi_full, corr = FALSE))
```

```{r, include = TRUE}
model_onset_diagnosis_basfi_reduced <- lmer(
  basfi ~ gender + hlab27status + age_at_onset +
    diagnosis + treatment_order + last_known_smoking_status +
    last_known_vaping_status +
    treatment_reclassified +
...
print(summary(model_onset_diagnosis_basfi_reduced, corr = FALSE))
```

```{r, include = TRUE}
anova(model_onset_diagnosis_basfi_full, model_onset_diagnosis_basfi_reduced)
```

```{r, include = TRUE}
anova(model_onset_diagnosis_basfi_full)
```

```{r, include = TRUE}
model_diag_diagnosis_basmi_full <- lmer(
  basmi_score ~ gender + hlab27status + age_at_diag +
    diagnosis + treatment_order + last_known_smoking_status +
    last_known_vaping_status + treatment_reclassified +
    age_at_diag * diagnosis +
    time_in_years + (time_in_years | ttas_id),
                     data = dataframe_sorted_and_reclassified_basmi,
                     control = lmerControl(optimizer = "bobyqa",
                                           optCtrl = list(maxfun = 2e5)))

print(summary(model_diag_diagnosis_basmi_full, corr = FALSE))
```

```{r, include = TRUE}
model_diag_diagnosis_basmi_reduced <- lmer(
  basmi_score ~ gender + hlab27status + age_at_diag +
    diagnosis + treatment_order + last_known_smoking_status +
    last_known_vaping_status +
    treatment_reclassified +
...
print(summary(model_diag_diagnosis_basmi_reduced, corr = FALSE))
```

```{r, include = TRUE}
anova(model_diag_diagnosis_basmi_full, model_diag_diagnosis_basmi_reduced)
```

```{r, include = TRUE}
anova(model_diag_diagnosis_basmi_full)
```

```{r, include = TRUE}
model_diag_diagnosis_basdai_full <- lmer(
  basdai ~ gender + hlab27status + age_at_diag +
    diagnosis + treatment_order + last_known_smoking_status +
    last_known_vaping_status + treatment_reclassified +
    age_at_diag * diagnosis +
    time_in_years + (time_in_years | ttas_id),
...
print(summary(model_diag_diagnosis_basdai_full, corr = FALSE))
```

```{r, include = TRUE}
model_diag_diagnosis_basdai_reduced <- lmer(
  basdai ~ gender + hlab27status + age_at_diag +
    diagnosis + treatment_order + last_known_smoking_status +
    last_known_vaping_status +
    treatment_reclassified +
...
print(summary(model_diag_diagnosis_basdai_reduced, corr = FALSE))
```

```{r, include = TRUE}
anova(model_diag_diagnosis_basdai_full, model_diag_diagnosis_basdai_reduced)
```

```{r, include = TRUE}
anova(model_diag_diagnosis_basdai_full)
```

```{r, include = TRUE}
model_diag_diagnosis_basfi_full <- lmer(
  basfi ~ gender + hlab27status + age_at_diag +
    diagnosis + treatment_order + last_known_smoking_status +
    last_known_vaping_status + treatment_reclassified + 
    age_at_diag * diagnosis +
    time_in_years + (time_in_years | ttas_id),
...
print(summary(model_diag_diagnosis_basfi_full, corr = FALSE))
```

```{r, include = TRUE}
model_diag_diagnosis_basfi_reduced <- lmer(
  basfi ~ gender + hlab27status + age_at_diag +
    diagnosis + treatment_order + last_known_smoking_status +
    last_known_vaping_status +
    treatment_reclassified +
...
print(summary(model_diag_diagnosis_basfi_reduced, corr = FALSE))
```

```{r, include = TRUE}
anova(model_diag_diagnosis_basfi_full, model_diag_diagnosis_basfi_reduced)
```

```{r, include = TRUE}
anova(model_diag_diagnosis_basfi_full)
```
