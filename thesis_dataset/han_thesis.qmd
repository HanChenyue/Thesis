...
# Step 6: Pivot the basmi_score to wide format
data_sorted <- data_sorted %>%
  group_by(ttas_id, treatment_period) %>%
  mutate(
...
data_wide <- data_sorted %>%
  pivot_wider(
    ...
    names_prefix = "basmi_score_", 
    values_from = basmi_score
  )

# write.csv(data_wide, "test_basmi_score_wide.csv", row.names = FALSE)

# Step 7: Filter on treatment day and inspect the result
final_data <- data_wide %>%
  filter(!is.na(x3)) %>%
  select(ttas_id, x3, hlab27status, diagnosis, date, stop_date_for_period_date_format, treatment, treatment_period, has_valid_treatment, basmi_score_before, overall_treatment_duration_in_days, starts_with("basmi_score_"))
# write.csv(data_sorted, "temp_check.csv", row.names = FALSE)
write.csv(final_data, "test_basmi_score.csv", row.names = FALSE)
```
