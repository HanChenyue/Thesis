...
```{r}
# Run an lmer model 
test_model_humira <- lmer(basmi_score ~ (1 | ttas_id), data = longitudinal_analysis_data_filtered %>% filter(treatment == "Humira"))
summary(test_model_humira)
```

```{r}
# Run an lmer model for Humira with diagnosis as a fixed effect for longitudinal_analysis_extreme_data
test_model_humira <- lmer(basmi_score ~ diagnosis + (1 | ttas_id), data = longitudinal_analysis_extreme_data %>% filter(treatment == "Humira"))
summary(test_model_humira)
```

```{r}
# Plot the data
test_model_1 <- lmer(basmi_score ~ treatment + treatment_window_weekly_time + age_at_basmi + gender + hlab27status + (1 | ttas_id), data = longitudinal_analysis_data_filtered)
...
# Fit a full model with all interactions of interest

test_basmi_model_1_data <- na.omit(longitudinal_analysis_data_filtered)
test_basmi_model_1 <- lmer(basmi_score ~ (gender + hlab27status + diagnosis + treatment + age_at_onset + age_at_diag
                  + age_at_basmi)^2 + (1 | ttas_id),
                  data = test_basmi_model_1_data, na.action = na.fail)

# Perform model selection based on AIC
test_basmi_model_1_dredge <-  dredge(test_basmi_model_1)
```

```{r}
# Fit a full model with all interactions of interest
test_basmi_model_2 <- lmer(basmi_score ~ (gender + hlab27status + diagnosis + treatment +
                  + age_at_basmi)^4 + (1 | ttas_id),
                  data = longitudinal_analysis_data_filtered, na.action = na.fail)

# Perform model selection based on AIC
test_basmi_model_2_dredge <-  dredge(test_basmi_model_2)

# 
# # View the top models
# model_selection[1:5]  # Show top 5 models
# 
