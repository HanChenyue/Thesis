...
```{r}
data_tt1 <- dataframe_sorted %>% 
  filter(ttas_id %in% c(1, 25))
  
# Run a test lmer model
test_model <- lmer(
  basmi_score ~ treatment + treatment_window_weekly_time + age_at_basmi + 
  (1 | ttas_id/true_treatment_period_unique_identifier),
  data = data_tt1
)
...
ggplot(data_tt1, aes(x = treatment_window_weekly_time, y = basmi_score,
  ...
  labs(title = "BASMI Score Over Time by Treatment Periods for ttas_id 1", 
       x = "Treatment window weekly time", 
       y = "BASMI score", 
