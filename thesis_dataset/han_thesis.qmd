...
data_sorted <- df %>%
  select(ttas_id, x3, gender, hlab27status, age_at_onset, age_at_diag, diagnosis, date, stop_date, basmi_score)
...
longitudinal_analysis_data <- data_sorted %>% 
  ...
  select(ttas_id, gender, hlab27status, age_at_onset, age_at_diag, diagnosis, treatment, treatment_period_unique_identifier, date, stop_date_for_period_date_format, overall_treatment_duration_in_days, treatment_window_weekly_time, basmi_score_before_treatment, basmi_score)
...
longitudinal_analysis_data_filtered <- longitudinal_analysis_data_with_zeroth %>%
...
```{r}
# Run an lmer model for Humira for longitudinal_analysis_extreme_data
test_model_humira <- lmer(basmi_score ~ (1 | ttas_id), data = longitudinal_analysis_extreme_data %>% filter(treatment == "Humira"))
...
```{r}
# Plot the data
test_1 <- ggplot(data = longitudinal_analysis_data_filtered %>% filter(treatment == "Humira"),
       aes(
