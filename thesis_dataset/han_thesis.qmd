...
# write.csv(data_sorted, "temp_check.csv", row.names = FALSE)
# write.csv(data_sorted, "before_final_treatment_data.csv", row.names = FALSE)
```
...
data_sorted <- data_sorted %>%
  ...
  mutate(
    ...
    # stop_date_for_period is identified as the maximum stop_date_unix within each treatment_period group. This assumes that each treatment period should only have one stop_date.
    stop_date_for_period = max(stop_date_unix, na.rm = TRUE),
    
    # If there is no stop_date but there in a period, stop_date_for_period is set to current date.
    stop_date_for_period = ifelse(is.na(stop_date_for_period), Sys.Date(), stop_date_for_period),
    
    # If the date is after the stop_date_for_period, reset treatment to NA for the row
    treatment = ifelse(!is.na(treatment) & date_unix > stop_date_for_period, NA_character_, treatment)
    
  ) %>%
  ungroup()


# Convert UNIX time back to date immediately after Step 3
data_sorted <- data_sorted %>%
  mutate(
    date = as.Date(as.POSIXct(date_unix, origin = "1970-01-01")),
    stop_date = as.Date(as.POSIXct(stop_date_unix, origin = "1970-01-01"))
  )

# # Step 2: Calculate basmi_score_before
# data_sorted <- data_sorted %>%
#   group_by(ttas_id) %>%
#   mutate(basmi_score_before = lag(basmi_score, default = NA_real_)) %>%
#   fill(basmi_score_before, .direction = "down") %>%
#   ungroup()
# 
# # Step 3: Create a unique treatment period identifier
# data_sorted <- data_sorted %>%
#   group_by(ttas_id) %>%
#   mutate(treatment_period = cumsum(!duplicated(treatment) & !is.na(treatment))) %>%
#   ungroup()
# 
# # Step 4: Identify the start and stop dates for each treatment period
# data_sorted <- data_sorted %>%
#   group_by(ttas_id, treatment, treatment_period) %>%
#   mutate(
#     treatment_start = min(date, na.rm = TRUE),
#     treatment_stop = case_when(
#       !is.na(stop_date) ~ stop_date,      # Use `stop_date` if available
#       is.na(stop_date) ~ Sys.Date(),      # If `stop_date` is `NA`, set to today's date
#       TRUE ~ max(date, na.rm = TRUE)      # Otherwise, use the max date within the treatment period
#     )
#   ) %>%
#   ungroup()
# 
# # Step 5: Filter on treatment day and inspect the result
# final_data <- data_sorted %>%
#   filter(!is.na(x3)) %>%
#   select(ttas_id, basmi_score, x3, hlab27status, diagnosis, date, stop_date, treatment, basmi_score_before, treatment_start, treatment_stop)

# write.csv(data_sorted, "temp_check.csv", row.names = FALSE)
write.csv(final_data, "basmi_score.csv", row.names = FALSE)
```
