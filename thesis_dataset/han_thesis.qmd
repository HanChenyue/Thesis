...
longitudinal_analysis_data <- data_sorted %>% 
  ...
  select(ttas_id, treatment, treatment_period_unique_identifier, has_valid_treatment, hlab27status, diagnosis, date, stop_date_for_period_date_format, overall_treatment_duration_in_days, treatment_window_weekly_time, basmi_score_before_treatment, basmi_score)
...
# Ensure ttas_id is a factor
longitudinal_analysis_data_with_zeroth$ttas_id <- as.factor(longitudinal_analysis_data_with_zeroth$ttas_id)
test_model <- lmer(basmi_score ~ (1 | ttas_id), data = longitudinal_analysis_data_with_zeroth)
summary(test_model)
```


```{r}
# Run an lmer model for Humira
test_model_humira <- lmer(basmi_score ~ (1 | ttas_id), data = longitudinal_analysis_data_with_zeroth %>% filter(treatment == "Humira"))
summary(test_model_humira)
```

```{r}
# Run an lmer model for Humira with diagnosis as a fixed effect
test_model_humira <- lmer(basmi_score ~ diagnosis + (1 | ttas_id), data = longitudinal_analysis_data_with_zeroth %>% filter(treatment == "Humira"))
summary(test_model_humira)
```

```{r}
# Plot the data
test_1 <- ggplot(data = longitudinal_analysis_data_with_zeroth %>% filter(treatment == "Humira"),
       aes(
         x = treatment_window_weekly_time,
         y = basmi_score,
         group = ttas_id,
         colour = "ttas_id"
         )) +
...
```{r}
test_1 <- ggplot(data = longitudinal_analysis_data_with_zeroth,
       aes(
         x = treatment_window_weekly_time,
         y = basmi_score,
         group = ttas_id,
  ...
  geom_line() +
  geom_point() +
  facet_wrap(~ treatment_period_unique_identifier, scales = "free_y")
