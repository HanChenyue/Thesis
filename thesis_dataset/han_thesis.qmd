...
prom_sheet_data <- prom_sheet_data %>%
  mutate(do_you_smoke = case_when(
    ...
    ttas_id == 7 & do_you_smoke == "Ex-smoke" ~ NA_character_,
...
prom_sheet_data <- prom_sheet_data %>%
  mutate(do_you_smoke = case_when(
    ...
    ttas_id == 72 & do_you_smoke == "Ex-smoke" ~ NA_character_,
...
prom_sheet_data_clean_smoke <- prom_sheet_data_clean_smoke %>%
  select(ttas_id, do_you_smoke, smoking_status)

write_csv(prom_sheet_data_clean_smoke, "test_checking_smoking_status_data.csv")
```
...
dataframe_sorted <- dataframe %>%
  select(ttas_id, x3, gender, hlab27status, age_at_basmi, diagnosis, date, stop_date, basmi_score)
...
dataframe_sorted <- dataframe_sorted %>%
  ...
  select(ttas_id, gender, hlab27status, age_at_basmi, diagnosis, treatment, true_treatment_period_unique_identifier, date, stop_date_for_period_date_format, overall_treatment_duration_in_days, treatment_window_weekly_time, time_in_days, time_in_weeks, time_in_months, time_in_years, basmi_score)
...
```{r}
model_11 <- lmer(basmi_score ~ gender + hlab27status + age_at_basmi + diagnosis + true_treatment_period_unique_identifier + treatment_reclassified*diagnosis + time_in_years + (time_in_years | ttas_id), data = dataframe_sorted_and_reclassified)
...
model_11_full <- lmer(basmi_score ~ gender + hlab27status + age_at_basmi + diagnosis + true_treatment_period_unique_identifier + treatment_reclassified*diagnosis + time_in_years + (time_in_years | ttas_id), data = dataframe_sorted_and_reclassified,
                         # control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))
                         control = lmerControl(optCtrl = list(maxfun = 2e5))
                      )
model_11_reduced <- lmer(basmi_score ~ gender + hlab27status + age_at_basmi + true_treatment_period_unique_identifier + treatment_reclassified + diagnosis + time_in_years + (time_in_years | ttas_id), data = dataframe_sorted_and_reclassified,
                         control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))
                         )
