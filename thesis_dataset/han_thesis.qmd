...
dataframe_sorted <- dataframe %>%
  select(ttas_id, x3, gender, hlab27status, age_at_basmi, diagnosis, date, stop_date, last_known_smoking_status, last_known_vaping_status, basmi_score, basdai, basfi, patient_global, back_pain_at_night, back_pain_anytime, as_qol, jenkins_sleep_1, jenkins_sleep_2, jenkins_sleep_3, jenkins_sleep_4, wpl, daily_activity_impairment, presentism, absenteeism, facit, margolis)
...
dataframe_sorted$presentism <- as.numeric(dataframe_sorted$presentism)
...
dataframe_sorted_1 <- dataframe_sorted %>%
...
write.csv(dataframe_sorted_1, "temp_dataframe_after_dataframe_sorted_for_analysis.csv", row.names = FALSE)

# Ignore if there is only one instance of row per treatment_period_unique_identifier in each ttas_id
dataframe_sorted_1 <- dataframe_sorted_1 %>%
  group_by(ttas_id, treatment_order) %>%
  filter(n() > 1) %>% # Keep only groups with more than one row after filtering out NAs
  ungroup()

write.csv(dataframe_sorted_1, "temp_dataframe_after_dataframe_sorted_for_analysis_filtered.csv", row.names = FALSE)
```

```{r}
# Reclassify the drugs
dataframe_sorted_and_reclassified <- dataframe_sorted_1 %>%
...
dataframe_sorted_and_reclassified$treatment_order <- as.factor(dataframe_sorted_and_reclassified$treatment_order)
...
# model_1 <- lmer(basmi_score ~ gender + hlab27status + age_at_basmi + diagnosis + treatment_reclassified + treatment_window_weekly_time + (1 | ttas_id/treatment_order), data = dataframe_sorted_and_reclassified)
# 
...
# model_2 <- lmer(basmi_score ~ gender + hlab27status + age_at_basmi + diagnosis + treatment_reclassified + treatment_window_weekly_time + (1 + treatment_window_weekly_time | ttas_id/treatment_order), data = dataframe_sorted_and_reclassified)
# model_3 <- lmer(basmi_score ~ gender + hlab27status + age_at_basmi + diagnosis + treatment_reclassified + treatment_window_weekly_time + (treatment_window_weekly_time | ttas_id/treatment_order), data = dataframe_sorted_and_reclassified)

# Model failed to converge. Model is nearly unidentifiable: very large eigenvalue
# model_4 <- lmer(basmi_score ~ gender + hlab27status + age_at_basmi + diagnosis + treatment_reclassified + treatment_window_weekly_time + (age_at_basmi | ttas_id/treatment_order), data = dataframe_sorted_and_reclassified)
```

```{r}
# model_5 <- lmer(basmi_score ~ gender + hlab27status + age_at_basmi + diagnosis + treatment_reclassified*gender + treatment_window_weekly_time + (1 | ttas_id/treatment_order), data = dataframe_sorted_and_reclassified)
# 
...
```{r}
# model_6 <- lmer(basmi_score ~ gender + hlab27status + age_at_basmi + diagnosis + treatment_reclassified*diagnosis + treatment_window_weekly_time + (1 | ttas_id/treatment_order), data = dataframe_sorted_and_reclassified)
# 
...
```
```{r}
# model_6_lmerTest <- lmerTest::lmer(basmi_score ~ gender + hlab27status + age_at_basmi + diagnosis + treatment_reclassified*diagnosis + treatment_window_weekly_time + (1 | ttas_id/treatment_order), data = dataframe_sorted_and_reclassified)
# 
...
# model_7 <- lmer(basmi_score ~ gender + hlab27status + age_at_basmi + diagnosis + treatment_reclassified*hlab27status + treatment_window_weekly_time + (1 | ttas_id/treatment_order), data = dataframe_sorted_and_reclassified)

# Fixed-effect model matrix is rank deficient so dropping 2 columns / coefficients
# model_8 <- lmer(basmi_score ~ gender + hlab27status + age_at_basmi + diagnosis + treatment_reclassified*gender*diagnosis + treatment_window_weekly_time + (1 | ttas_id/treatment_order), data = dataframe_sorted_and_reclassified)
```

```{r}
# model_9 <- lmer(basmi_score ~ gender + hlab27status +  diagnosis * treatment_reclassified + age_at_basmi + treatment_window_weekly_time + (treatment_window_weekly_time | ttas_id/treatment_order), data = dataframe_sorted_and_reclassified)
# 
...
```{r}
# model_1 <- lmer(basmi_score ~ gender + hlab27status + last_known_smoking_status + last_known_vaping_status + age_at_basmi + diagnosis + treatment_order + treatment_reclassified*diagnosis + time_in_years + (time_in_years | ttas_id), data = dataframe_sorted_and_reclassified)
# 
# model_1
...
# model_12 <- lmer(basmi_score ~ gender + hlab27status + age_at_basmi + diagnosis + treatment_order + treatment_reclassified*diagnosis + time_in_years + (1 | ttas_id), data = dataframe_sorted_and_reclassified)
# 
...
# Make sure that the reference level for treatment_reclassified is "Standard Non-High Cost Drug Treatment"
dataframe_sorted_and_reclassified$treatment_reclassified <- relevel(dataframe_sorted_and_reclassified$treatment_reclassified, ref = "Standard Non-High Cost Drug Treatment")

# Make sure that the reference level for hlba27status is "Unknown"


# Conduct an F-test for a full-model and reduced model for model_11
model_1_full <- lmer(basmi_score ~ gender + hlab27status + age_at_basmi + diagnosis + treatment_order + last_known_smoking_status + last_known_vaping_status + treatment_reclassified*diagnosis + time_in_years + (time_in_years | ttas_id), data = dataframe_sorted_and_reclassified,
                         control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))
                      )
model_1_reduced <- lmer(basmi_score ~ gender + hlab27status + age_at_basmi + treatment_order + last_known_smoking_status + last_known_vaping_status + treatment_reclassified + diagnosis + time_in_years + (time_in_years | ttas_id), data = dataframe_sorted_and_reclassified,
                         control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))
                         )
```
...
```{r}
model_1_full
model_1_full_vif <- vif(model_1_full)
print(summary(model_1_full, corr = TRUE))
model_1_full_vif
```
...
```{r}
anova(model_1_full, model_1_reduced)
```
