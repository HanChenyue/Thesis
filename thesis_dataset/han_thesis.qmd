...
dataframe_sorted <- dataframe_sorted %>%
  ...
  select(ttas_id, gender, hlab27status, age_at_basmi, diagnosis, treatment, true_treatment_period_unique_identifier, date, stop_date_for_period_date_format, overall_treatment_duration_in_days, treatment_window_weekly_time, basmi_score)
...
ggplot(data_tt1, aes(x = treatment_window_weekly_time, y = basmi_score,
  ...
  labs(title = "BASMI Score Over Time by Treatment Periods for Patient 1 to Patient 4", 
       x = "Treatment window (Weekly Time Scale)", 
       y = "BASMI Score", 
...
```{r}
model_9 <- lmer(basmi_score ~ gender + hlab27status +  age_at_basmi * treatment_reclassified + diagnosis + treatment_window_weekly_time + (1 | ttas_id/true_treatment_period_unique_identifier), data = dataframe_sorted_and_reclassified)
