...
```{r, include=TRUE, fig.width= 15, fig.height = 15}
...
```{r, include=TRUE, fig.height=15, fig.width=15}
...
# Jenkins Sleep only
# General longitudinal analysis for those with and without drugs
dataframe_sorted_jenkins_sleep <- dataframe_sorted %>%
  filter(!is.na(jenkins_sleep)) %>%
  select(ttas_id, gender, hlab27status, age_at_prom, diagnosis, treatment, last_known_smoking_status, last_known_vaping_status, treatment_order, date, stop_date_for_period_date_format, overall_treatment_duration_in_days, treatment_window_weekly_time, time_in_days, time_in_weeks, time_in_months, time_in_years, jenkins_sleep_1, jenkins_sleep_2, jenkins_sleep_3, jenkins_sleep_4)

write.csv(dataframe_sorted_jenkins_sleep, "temp_dataframe_after_dataframe_sorted_for_analysis_jenkins_sleep.csv", row.names = FALSE)
```

```{r, include=FALSE}
# Ignore if there is only one instance of row per treatment_period_unique_identifier in each ttas_id
dataframe_sorted_jenkins_sleep <- dataframe_sorted_jenkins_sleep %>%
  group_by(ttas_id, treatment_order) %>%
  filter(n() > 1) %>% # Keep only groups with more than one row after filtering out NAs
  ungroup()

write.csv(dataframe_sorted_jenkins_sleep, "temp_dataframe_after_dataframe_sorted_for_analysis_filtered.csv", row.names = FALSE)

# Reclassify the drugs
dataframe_sorted_and_reclassified_jenkins_sleep <- dataframe_sorted_jenkins_sleep %>%
  mutate(treatment_reclassified = case_when(
    treatment %in% c("Amgevita", "Humira", "Imraldi", "Yuflyma", "Benepali", "Enbrel", "Simponi", "Remicade", "Remsima", "Cimzia") ~ "Anti-TNF",
    treatment %in% c("Cosentyx", "Taltz", "Bimzelx") ~ "Anti-IL17",
    treatment %in% c("Rinvoq", "Xeljanz") ~ "JAK Inhibitor",
...
write.csv(dataframe_sorted_and_reclassified_jenkins_sleep, "temp_dataframe_after_dataframe_sorted_for_analysis_filtered_with_treatment_category_jenkins_sleep.csv", row.names = FALSE)

# If diagnosis is "Undifferentiated spondyloarthritis (uSpA) (17 ttas_id)" and "Axial SpA- No subgroup" (1 ttas_id), remove it as most of them are on standard treatment and is their representation is limited
dataframe_sorted_and_reclassified_jenkins_sleep <- dataframe_sorted_and_reclassified_jenkins_sleep %>%
  filter(diagnosis != "Undifferentiated spondyloarthritis (uSpA)" & diagnosis != "Axial SpA- No subgroup")

write.csv(dataframe_sorted_and_reclassified_jenkins_sleep, "temp_dataframe_after_dataframe_sorted_for_analysis_filtered_with_treatment_category_and_diagnosis_jenkins_sleep.csv", row.names = FALSE)
...
dataframe_sorted_and_reclassified_jenkins_sleep$ttas_id <- as.factor(dataframe_sorted_and_reclassified_jenkins_sleep$ttas_id)
dataframe_sorted_and_reclassified_jenkins_sleep$gender <- as.factor(dataframe_sorted_and_reclassified_jenkins_sleep$gender)
dataframe_sorted_and_reclassified_jenkins_sleep$hlab27status <- as.factor(dataframe_sorted_and_reclassified_jenkins_sleep$hlab27status)
dataframe_sorted_and_reclassified_jenkins_sleep$age_at_prom <- as.numeric(dataframe_sorted_and_reclassified_jenkins_sleep$age_at_prom)
dataframe_sorted_and_reclassified_jenkins_sleep$diagnosis <- as.factor(dataframe_sorted_and_reclassified_jenkins_sleep$diagnosis)
dataframe_sorted_and_reclassified_jenkins_sleep$treatment <- as.factor(dataframe_sorted_and_reclassified_jenkins_sleep$treatment)
dataframe_sorted_and_reclassified_jenkins_sleep$treatment_order <- as.factor(dataframe_sorted_and_reclassified_jenkins_sleep$treatment_order)
dataframe_sorted_and_reclassified_jenkins_sleep$treatment_reclassified <- as.factor(dataframe_sorted_and_reclassified_jenkins_sleep$treatment_reclassified)
dataframe_sorted_and_reclassified_jenkins_sleep$date <- as.Date(dataframe_sorted_and_reclassified_jenkins_sleep$date)
dataframe_sorted_and_reclassified_jenkins_sleep$stop_date_for_period_date_format <- as.Date(dataframe_sorted_and_reclassified_jenkins_sleep$stop_date_for_period_date_format)
dataframe_sorted_and_reclassified_jenkins_sleep$overall_treatment_duration_in_days <- as.numeric(dataframe_sorted_and_reclassified_jenkins_sleep$overall_treatment_duration_in_days)
dataframe_sorted_and_reclassified_jenkins_sleep$treatment_window_weekly_time <- as.numeric(dataframe_sorted_and_reclassified_jenkins_sleep$treatment_window_weekly_time)
dataframe_sorted_and_reclassified_jenkins_sleep$jenkins_sleep_1 <- as.numeric(dataframe_sorted_and_reclassified_jenkins_sleep$jenkins_sleep_1)
dataframe_sorted_and_reclassified_jenkins_sleep$jenkins_sleep_2 <- as.numeric(dataframe_sorted_and_reclassified_jenkins_sleep$jenkins_sleep_2)
dataframe_sorted_and_reclassified_jenkins_sleep$jenkins_sleep_3 <- as.numeric(dataframe_sorted_and_reclassified_jenkins_sleep$jenkins_sleep_3)
dataframe_sorted_and_reclassified_jenkins_sleep$jenkins_sleep_4 <- as.numeric(dataframe_sorted_and_reclassified_jenkins_sleep$jenkins_sleep_4)
dataframe_sorted_and_reclassified_jenkins_sleep$time_in_days <- as.numeric(dataframe_sorted_and_reclassified_jenkins_sleep$time_in_days)
dataframe_sorted_and_reclassified_jenkins_sleep$time_in_weeks <- as.numeric(dataframe_sorted_and_reclassified_jenkins_sleep$time_in_weeks)
dataframe_sorted_and_reclassified_jenkins_sleep$time_in_months <- as.numeric(dataframe_sorted_and_reclassified_jenkins_sleep$time_in_months)
dataframe_sorted_and_reclassified_jenkins_sleep$time_in_years <- as.numeric(dataframe_sorted_and_reclassified_jenkins_sleep$time_in_years)
dataframe_sorted_and_reclassified_jenkins_sleep$last_known_smoking_status <- as.factor(dataframe_sorted_and_reclassified_jenkins_sleep$last_known_smoking_status)
dataframe_sorted_and_reclassified_jenkins_sleep$last_known_vaping_status <- as.factor(dataframe_sorted_and_reclassified_jenkins_sleep$last_known_vaping_status)
```

```{r, include=FALSE}
# Make sure that the reference level for treatment_reclassified is
# "Standard Non-High Cost Drug Treatment"
dataframe_sorted_and_reclassified_jenkins_sleep$treatment_reclassified <-
  relevel(dataframe_sorted_and_reclassified_jenkins_sleep$treatment_reclassified,
          ref = "Standard Non-High Cost Drug Treatment")

# Make sure that the reference level for hlba27status is "undiagnosed"
dataframe_sorted_and_reclassified_jenkins_sleep$hlab27status <-
  relevel(dataframe_sorted_and_reclassified_jenkins_sleep$hlab27status,
          ref = "undiagnosed")

# Make sure that the reference level for diagnosis is "Axial SpA- nr-axSpA"
dataframe_sorted_and_reclassified_jenkins_sleep$diagnosis <-
  relevel(dataframe_sorted_and_reclassified_jenkins_sleep$diagnosis,
          ref = "Axial SpA- AS")

# Make sure that the reference level for treatment_order is 1
dataframe_sorted_and_reclassified_jenkins_sleep$treatment_order <-
  relevel(dataframe_sorted_and_reclassified_jenkins_sleep$treatment_order,
          ref = 1)

# Make sure that the reference level for last_known_smoking_status is
# "no information"
dataframe_sorted_and_reclassified_jenkins_sleep$last_known_smoking_status <-
  relevel(dataframe_sorted_and_reclassified_jenkins_sleep$last_known_smoking_status,
          ref = "no information")

# Make sure that the reference level for last_known_vaping_status is
# "no information"
dataframe_sorted_and_reclassified_jenkins_sleep$last_known_vaping_status <-
  relevel(dataframe_sorted_and_reclassified_jenkins_sleep$last_known_vaping_status,
          ref = "no information")
```
...
model_treatment_diagnosis_jenkins_sleep_1_full <- lmer(
  jenkins_sleep_1 <- gender + hlab27status + age_at_prom +
    ...
    treatment_reclassified * diagnosis +
    time_in_years + (time_in_years | ttas_id),
                     data = dataframe_sorted_and_reclassified_jenkins_sleep,
                     control = lmerControl(optimizer = "bobyqa",
                                           optCtrl = list(maxfun = 2e5)))
