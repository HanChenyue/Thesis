...
# 2. However, if the hlba27status is "NA", replace it with "undiagnosed"
demographics_sheet_data <- demographics_sheet_data %>%
...
dataframe_sorted <- dataframe %>%
  select(ttas_id, x3, gender, hlab27status, age_at_basmi, diagnosis, date, stop_date, last_known_smoking_status, last_known_vaping_status, basmi_score)
...
dataframe_sorted <- dataframe_sorted %>%
  group_by(ttas_id) %>%
  mutate(
    true_treatment_period_unique_identifier = cumsum(treatment != lag(treatment, default = first(treatment)) | is.na(lag(treatment)))
  ) %>%
  ungroup()

write.csv(dataframe_sorted, "temp_dataframe_after_data_sorted_5.csv", row.names = FALSE)

# Step 6: Create a new stop_date for each true_treatment_period_unique_identifier if there is no stop_date for treatment = "standard_non-high_cost_drug_treatment"
# If true_treatment_period_unique_identifier precedes another one, set stop_date_for_period to the next start_date_unix - 1
# If true_treatment_period_unique_identifier is the last one, set stop_date_for_period to current date
dataframe_sorted<- dataframe_sorted %>% 
  group_by(ttas_id) %>%
...
# Step 7: Fill the stop_date_for_period within each true_treatment_period_unique_identifier
dataframe_sorted<- dataframe_sorted%>%
  group_by(ttas_id, true_treatment_period_unique_identifier) %>%
...
# Step 9: Calculate weekly time within each true_treatment_period_unique_identifier
# Ensure date and stop_date_for_period_date_format are in Date format
dataframe_sorted$date <- as.Date(dataframe_sorted$date)
...
dataframe_sorted <- dataframe_sorted %>%
  ...
  select(ttas_id, gender, hlab27status, age_at_basmi, diagnosis, treatment, last_known_smoking_status, last_known_vaping_status, true_treatment_period_unique_identifier, date, stop_date_for_period_date_format, overall_treatment_duration_in_days, treatment_window_weekly_time, time_in_days, time_in_weeks, time_in_months, time_in_years, basmi_score)

write.csv(dataframe_sorted, "temp_dataframe_after_dataframe_sorted_for_analysis.csv", row.names = FALSE)
...
write.csv(dataframe_sorted, "temp_dataframe_after_dataframe_sorted_for_analysis_filtered.csv", row.names = FALSE)
```

```{r}
# Reclassify the drugs
dataframe_sorted_and_reclassified <- dataframe_sorted %>%
...
write.csv(dataframe_sorted_and_reclassified, "temp_dataframe_after_dataframe_sorted_for_analysis_filtered_with_treatment_category.csv", row.names = FALSE)
```

```{r}
# If diagnosis is "Undifferentiated spondyloarthritis (uSpA) (17 ttas_id)" and "Axial SpA- No subgroup" (1 ttas_id), remove it as all of them are on standard treatment and is their representation is limited
dataframe_sorted_and_reclassified <- dataframe_sorted_and_reclassified %>%
  filter(diagnosis != "Undifferentiated spondyloarthritis (uSpA)" & diagnosis != "Axial SpA- No subgroup")

write.csv(dataframe_sorted_and_reclassified, "temp_dataframe_after_dataframe_sorted_for_analysis_filtered_with_treatment_category_and_diagnosis.csv", row.names = FALSE)

# If treatment_reclassified is "drop", remove the row
dataframe_sorted_and_reclassified <- dataframe_sorted_and_reclassified %>%
  filter(treatment_reclassified != "drop")

write.csv(dataframe_sorted_and_reclassified, "temp_dataframe_after_dataframe_sorted_for_analysis_filtered_with_treatment_category_and_diagnosis_and_treatment_reclassified.csv", row.names = FALSE)
```

```{r, fig.height = 8}
data_tt1 <- dataframe_sorted_and_reclassified %>% 
...
ggplot(data_tt1, aes(x = time_in_years, y = basmi_score,
                     colour = interaction(factor(ttas_id),
                                          factor(true_treatment_period_unique_identifier)
                                          ))) +
...
```{r}
# Ensure that dataframe_sorted_and_reclassified has the correct data types
dataframe_sorted_and_reclassified$ttas_id <- as.factor(dataframe_sorted_and_reclassified$ttas_id)
dataframe_sorted_and_reclassified$gender <- as.factor(dataframe_sorted_and_reclassified$gender)
dataframe_sorted_and_reclassified$hlab27status <- as.factor(dataframe_sorted_and_reclassified$hlab27status)
dataframe_sorted_and_reclassified$age_at_basmi <- as.numeric(dataframe_sorted_and_reclassified$age_at_basmi)
dataframe_sorted_and_reclassified$diagnosis <- as.factor(dataframe_sorted_and_reclassified$diagnosis)
dataframe_sorted_and_reclassified$treatment <- as.factor(dataframe_sorted_and_reclassified$treatment)
dataframe_sorted_and_reclassified$true_treatment_period_unique_identifier <- as.factor(dataframe_sorted_and_reclassified$true_treatment_period_unique_identifier)
dataframe_sorted_and_reclassified$treatment_reclassified <- as.factor(dataframe_sorted_and_reclassified$treatment_reclassified)
dataframe_sorted_and_reclassified$date <- as.Date(dataframe_sorted_and_reclassified$date)
dataframe_sorted_and_reclassified$stop_date_for_period_date_format <- as.Date(dataframe_sorted_and_reclassified$stop_date_for_period_date_format)
dataframe_sorted_and_reclassified$overall_treatment_duration_in_days <- as.numeric(dataframe_sorted_and_reclassified$overall_treatment_duration_in_days)
dataframe_sorted_and_reclassified$treatment_window_weekly_time <- as.numeric(dataframe_sorted_and_reclassified$treatment_window_weekly_time)
dataframe_sorted_and_reclassified$basmi_score <- as.numeric(dataframe_sorted_and_reclassified$basmi_score)
dataframe_sorted_and_reclassified$time_in_days <- as.numeric(dataframe_sorted_and_reclassified$time_in_days)
dataframe_sorted_and_reclassified$time_in_weeks <- as.numeric(dataframe_sorted_and_reclassified$time_in_weeks)
dataframe_sorted_and_reclassified$time_in_months <- as.numeric(dataframe_sorted_and_reclassified$time_in_months)
dataframe_sorted_and_reclassified$time_in_years <- as.numeric(dataframe_sorted_and_reclassified$time_in_years)
dataframe_sorted_and_reclassified$last_known_smoking_status <- as.factor(dataframe_sorted_and_reclassified$last_known_smoking_status)
dataframe_sorted_and_reclassified$last_known_vaping_status <- as.factor(dataframe_sorted_and_reclassified$last_known_vaping_status)
```


```{r}
# Run an lmer model for dataframe_sorted_and_reclassified
model_1 <- lmer(basmi_score ~ gender + hlab27status + age_at_basmi + diagnosis + treatment_reclassified + treatment_window_weekly_time + (1 | ttas_id/true_treatment_period_unique_identifier), data = dataframe_sorted_and_reclassified)
...
# model_2 <- lmer(basmi_score ~ gender + hlab27status + age_at_basmi + diagnosis + treatment_reclassified + treatment_window_weekly_time + (1 + treatment_window_weekly_time | ttas_id/true_treatment_period_unique_identifier), data = dataframe_sorted_and_reclassified)
# model_3 <- lmer(basmi_score ~ gender + hlab27status + age_at_basmi + diagnosis + treatment_reclassified + treatment_window_weekly_time + (treatment_window_weekly_time | ttas_id/true_treatment_period_unique_identifier), data = dataframe_sorted_and_reclassified)

# Model failed to converge. Model is nearly unidentifiable: very large eigenvalue
# model_4 <- lmer(basmi_score ~ gender + hlab27status + age_at_basmi + diagnosis + treatment_reclassified + treatment_window_weekly_time + (age_at_basmi | ttas_id/true_treatment_period_unique_identifier), data = dataframe_sorted_and_reclassified)
```

```{r}
model_5 <- lmer(basmi_score ~ gender + hlab27status + age_at_basmi + diagnosis + treatment_reclassified*gender + treatment_window_weekly_time + (1 | ttas_id/true_treatment_period_unique_identifier), data = dataframe_sorted_and_reclassified)
...
```{r}
model_6 <- lmer(basmi_score ~ gender + hlab27status + age_at_basmi + diagnosis + treatment_reclassified*diagnosis + treatment_window_weekly_time + (1 | ttas_id/true_treatment_period_unique_identifier), data = dataframe_sorted_and_reclassified)
...
```
```{r}
model_6_lmerTest <- lmerTest::lmer(basmi_score ~ gender + hlab27status + age_at_basmi + diagnosis + treatment_reclassified*diagnosis + treatment_window_weekly_time + (1 | ttas_id/true_treatment_period_unique_identifier), data = dataframe_sorted_and_reclassified)
...
# qqnorm(ranef_model_6$ttas_id:true_treatment_period_unique_identifier[,1])
# qqline(ranef_model_6$ttas_id:true_treatment_period_unique_identifier[,1])
```
...
plot(dataframe_sorted_and_reclassified$age_at_basmi, residuals_model_6, 
...
plot(dataframe_sorted_and_reclassified$treatment_window_weekly_time, residuals_model_6,
...
# model_7 <- lmer(basmi_score ~ gender + hlab27status + age_at_basmi + diagnosis + treatment_reclassified*hlab27status + treatment_window_weekly_time + (1 | ttas_id/true_treatment_period_unique_identifier), data = dataframe_sorted_and_reclassified)

# Fixed-effect model matrix is rank deficient so dropping 2 columns / coefficients
# model_8 <- lmer(basmi_score ~ gender + hlab27status + age_at_basmi + diagnosis + treatment_reclassified*gender*diagnosis + treatment_window_weekly_time + (1 | ttas_id/true_treatment_period_unique_identifier), data = dataframe_sorted_and_reclassified)
```

```{r}
model_9 <- lmer(basmi_score ~ gender + hlab27status +  diagnosis * treatment_reclassified + age_at_basmi + treatment_window_weekly_time + (treatment_window_weekly_time | ttas_id/true_treatment_period_unique_identifier), data = dataframe_sorted_and_reclassified)
...
```
```{r}
model_10 <- lmer(basmi_score ~ gender + hlab27status + age_at_basmi + diagnosis + treatment_reclassified + poly(treatment_window_weekly_time, 2) + treatment_window_weekly_time * treatment_reclassified + (1 | ttas_id/true_treatment_period_unique_identifier)
, data = dataframe_sorted_and_reclassified)
```

```{r}
model_11 <- lmer(basmi_score ~ gender + hlab27status + last_known_smoking_status + last_known_vaping_status + age_at_basmi + diagnosis + true_treatment_period_unique_identifier + treatment_reclassified*diagnosis + time_in_years + (time_in_years | ttas_id), data = dataframe_sorted_and_reclassified)
...
```{r}
model_12 <- lmer(basmi_score ~ gender + hlab27status + age_at_basmi + diagnosis + true_treatment_period_unique_identifier + treatment_reclassified*diagnosis + time_in_years + (1 | ttas_id), data = dataframe_sorted_and_reclassified)
...
# Conduct an F-test for a full-model and reduced model for model_11
model_11_full <- lmer(basmi_score ~ gender + hlab27status + age_at_basmi + diagnosis + true_treatment_period_unique_identifier + last_known_smoking_status + last_known_vaping_status + treatment_reclassified*diagnosis + time_in_years + (time_in_years | ttas_id), data = dataframe_sorted_and_reclassified,
                         # control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))
                         control = lmerControl(optCtrl = list(maxfun = 2e5))
                      )
model_11_reduced <- lmer(basmi_score ~ gender + hlab27status + age_at_basmi + true_treatment_period_unique_identifier + last_known_smoking_status + last_known_vaping_status + treatment_reclassified + diagnosis + time_in_years + (time_in_years | ttas_id), data = dataframe_sorted_and_reclassified,
                         control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))
                         )
```
...
```{r}
anova(model_11_full, model_11_reduced)
```

```{r}
# Check distribution of response, Y
