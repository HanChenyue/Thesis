...
analysis_data <- final_data %>% 
  filter(!is.na(x3)) %>%
  select(ttas_id, treatment, x3, hlab27status, diagnosis, date, stop_date, treatment_period, has_valid_treatment, basmi_score_before, overall_treatment_duration_in_days, starts_with("basmi_score_"))
```
